...
import SubmarineConstants from "../model/SubmarineConstants";
import SubmarineState from "../model/SubmarineState";
...
class Phyiscs {
    /**
   * The environment in which the physics simulation takes place.
   * @private
   */
    private env: Environment;

    /**
     * Time step interval for simulation updates.
     * @private
     */
    private dt: number;

    /**
     * Simulator instance managing the overall simulation environment.
     * @private
     */
    private simulator: Simulator;

    /**
     * Arrow helpers used for visualizing vectors in the simulator.
     * @private
     */
    private arrowHelper: { [name: string]: THREE.ArrowHelper };

    /**
     * Current submarine instance being simulated.
     * @private
     */
    private submarine: Submarine;

    /**
     * Constants defining physical parameters of the current submarine.
     * @private
     */
    private submarineConstant: SubmarineConstants;

    /**
     * Current state (position, orientation, velocity, etc.) of the submarine.
     * @private
     */
    private submarineState: SubmarineState;

    /**
     * Mesh representing the ellipsoid shape of the submarine for CSG operations.
     * @private
     */
    private submarineEllipsoidMesh!: THREE.Mesh;

    /**
     * Volume of the ellipsoid mesh representing the submarine.
     * @private
     */
    private submarineEllipsoidVolume!: number;

    /**
     * Mesh representing the surface box used in CSG operations.
     * @private
     */
    private surfaceBoxMesh!: THREE.Mesh;

    /**
     * Mesh representing the cutting plane used in CSG operations.
     * @private
     */
  ...
  */
   ...
   */
    calculatePlaneForce(coefficient: number, area: number, angle: number): number {
        const waterDensity = this.env.getWaterDensity()
   ...
   */
    initSubmarineEllipsoidMesh(): void {
     ...
     */
    initSurfaceBoxMesh(): void {
        const surfaceBox = new THREE.Mesh(
   ...
   */
    updateSubmarineEllipsoidMeshPositionAndOrietntation(): void {
        const currentPosition = this.submarineState.getCurrentPosition();
     ...
     */
    drawVectorAsArrow(vector: THREE.Vector3, name: string, color: THREE.ColorRepresentation) {
     ...
     */
    calculatePolygonArea(vertices: THREE.BufferAttribute | THREE.InterleavedBufferAttribute): number {
        let area = 0;
   ...
   */
    private getVolume(geometry: THREE.BufferGeometry): number {
